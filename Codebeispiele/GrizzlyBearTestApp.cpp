...
public:
	...
	virtual GrizzlyBear::ModuleState on_module_init() override
	{
		...
		m_Window = GrizzlyBear::Ref<GrizzlyBear::Window>
		(GrizzlyBear::Window::create("TestApp", 1280, 720));
	...
	virtual bool on_startup() override
	{
		...
		m_Window->on_key(GrizzlyBear::KeyEvent::PRESSED, 
		[&](const GrizzlyBear::KeyCode& e)->bool
		{
			...
			if (e.KEY_CODE == GRIZZLY_KEY_V && 
			m_Window->is_key_pressed(GRIZZLY_KEY_LEFT_SHIFT))
			{
				m_Window->set_v_sync(GrizzlyBear::VsyncMode::TOGGLE);
				GRIZZLY_CLIENT_LOG(GrizzlyBear::LogLevel::Warn, "Toggled Vsync to: {0}",
				 m_Window->get_v_sync());
			}
		...
		scene->set_template_entity<GrizzlyBear::CompMesh2D, 
		GrizzlyBear::CompTransformation2D, GrizzlyBear::CompScript>("Player",
		{ 
	...
	{
		m_GuiRenderer->begin_scene(m_GuiCamera->get_projection(), 
		m_GuiCamera->calc_view());
